 ...
 * @since 0.5
 */
public abstract class AbstractPebldsTreaty implements Serializable, IAbstractEntity {

    private static final Logger log = Logger.getLogger(AbstractPebldsTreaty.class.getName());
    /**
     * Name of the collection as it appears in the OData metadata and response
     */
    public static final String COLLECTION_NAME = "PebldsTreaties";
    ...
    public static List<EdmProperty> getPrimitiveProperties() {
        ...
        properties.add(new EdmProperty("logoMedium", EdmType.STRING, true));
        properties.add(new EdmProperty("odataName", EdmType.STRING, false));

        return properties;
    }

    /**
     * This method works the same as {@link org.informea.odata.pojo.AbstractPebldsTreaty#getSchema(String)}, but for a list
     * of entities.
     ...
     */
    public static List<OEntity> asEntities(final EdmEntitySet ees, List<PebldsTreaty> entities) {
        return Enumerable.create(entities).select(new Func1<PebldsTreaty, OEntity>() {

            @Override
            public OEntity apply(PebldsTreaty entityA) {
    ...
    public static EdmEntityType getEntityType(String namespace) {
        return new EdmEntityType(namespace, null, "PebldsTreaty", null, Enumerable.create("id").toList(), getPrimitiveProperties(), null);
    }
