 ...
 * @since 0.5
 */
public abstract class AbstractPebldsProject implements Serializable, IAbstractEntity {

    private static final Logger log = Logger.getLogger(AbstractPebldsProject.class.getName());
    /**
     * Name of the collection as it appears in the OData metadata and response
     */
    public static final String COLLECTION_NAME = "PebldsProjects";
    public static final String NAV_PROP_TREATY = "project_treaties";
    public static final String NAV_PROP_COUNTRY = "project_countries";
    public static final String NAV_PROP_FILES = "project_files";
    ...
    public static List<EdmProperty> getPrimitiveProperties() {
        ...
        properties.add(new EdmProperty("objective", EdmType.STRING, true));
        properties.add(new EdmProperty("description", EdmType.STRING, false));
        ...
        properties.add(new EdmProperty("outcome", EdmType.STRING, true));
        properties.add(new EdmProperty("fundingAgency", EdmType.STRING, true));
        properties.add(new EdmProperty("budget", EdmType.DOUBLE, true));
        properties.add(new EdmProperty("startDate", EdmType.DATETIME, false, null, true, false, null, null, "text", "true", null, null));
        properties.add(new EdmProperty("endDate", EdmType.DATETIME, true, null, true, false, null, null, "text", "true", null, null));
        return properties;
    }

    /**
     * This method works the same as {@link org.informea.odata.pojo.AbstractPebldsProject#getSchema(String)}, but for a list
     * of entities.
     ...
     */
    public static List<OEntity> asEntities(final EdmEntitySet ees, List<AbstractPebldsProject> entities) {
        return Enumerable.create(entities).select(new Func1<AbstractPebldsProject, OEntity>() {

            @Override
            public OEntity apply(AbstractPebldsProject entityA) {
    ...
    public static EdmEntityType getEntityType(String namespace) {
        return new EdmEntityType(namespace, null, "PebldsProject", null, Enumerable.create("id").toList(), getPrimitiveProperties(), null);
    }
    ...
    public static EdmSchema getSchema(String namespace) {
        ...
        EdmEntityType eetTreaty = AbstractPebldsTreaty.getEntityType(namespace);
        entityTypes.add(eetTreaty);
        String propertyName = NAV_PROP_TREATY;
        ...
        EdmAssociation assocTreaty = new EdmAssociation(namespace, null,
                assocName,
        ...
        lAssoc.add(new EdmAssociationSet(assocName, assocTreaty,
        ...
        EdmNavigationProperty np = new EdmNavigationProperty(propertyName, assocTreaty, assocTreaty.end1, assocTreaty.end2);

        eet.navigationProperties.add(np);

        assocList.addAll(Enumerable.create(assocTreaty).toList());

        //nav country
        EdmEntityType eetCountry = AbstractPebldsCountry.getEntityType(namespace);
        entityTypes.add(eetCountry);
        propertyName = NAV_PROP_COUNTRY;

        EdmEntitySet eesCountry = new EdmEntitySet(propertyName, eetCountry);
        entitySets.add(eesCountry);

        assocName = String.format("FK_%s_%s", propertyName, COLLECTION_NAME);
        EdmAssociation assocCountry = new EdmAssociation(namespace, null,
                assocName,
                new EdmAssociationEnd(COLLECTION_NAME, eet, EdmMultiplicity.ZERO_TO_ONE),
                new EdmAssociationEnd(propertyName, eetCountry, EdmMultiplicity.MANY));

        lAssoc.add(new EdmAssociationSet(assocName, assocCountry,
                new EdmAssociationSetEnd(new EdmAssociationEnd(COLLECTION_NAME, eet, EdmMultiplicity.ZERO_TO_ONE), ees),
                new EdmAssociationSetEnd(new EdmAssociationEnd(propertyName, eetCountry, EdmMultiplicity.MANY), eesCountry)));

        np = new EdmNavigationProperty(propertyName, assocCountry, assocCountry.end1, assocCountry.end2);

        eet.navigationProperties.add(np);

        assocList.addAll(Enumerable.create(assocCountry).toList());

        //nav File
        EdmEntityType eetFile = AbstractPebldsWpfbFile.getEntityType(namespace);
        entityTypes.add(eetFile);
        propertyName = NAV_PROP_FILES;
        ...
        np = new EdmNavigationProperty(propertyName, assocFile, assocFile.end1, assocFile.end2);
