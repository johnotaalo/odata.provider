 ...
 * @since 0.5
 */
public abstract class AbstractPebldsTechnicalReport implements Serializable, IAbstractEntity {

    private static final Logger log = Logger.getLogger(AbstractPebldsTechnicalReport.class.getName());
    /**
     * Name of the collection as it appears in the OData metadata and response
     */
    public static final String COLLECTION_NAME = "PebldsTechnicalReports";
    public static final String NAV_PROP_FILES = "technical_report_files";
    public static final String NAV_PROP_TOPICS = "technical_report_topics";
    ...
    /**
     * This method works the same as {@link org.informea.odata.pojo.AbstractPebldsTechnicalReport#getSchema(String)}, but for a list
     * of entities.
     ...
     */
    public static List<OEntity> asEntities(final EdmEntitySet ees, List<AbstractPebldsTechnicalReport> entities) {
        return Enumerable.create(entities).select(new Func1<AbstractPebldsTechnicalReport, OEntity>() {

            @Override
            public OEntity apply(AbstractPebldsTechnicalReport entityA) {
    ...
    public static EdmEntityType getEntityType(String namespace) {
        return new EdmEntityType(namespace, null, "PebldsTechnicalReport", null, Enumerable.create("id").toList(), getPrimitiveProperties(), null);
    }
    ...
    public static EdmSchema getSchema(String namespace) {
        ...
        //nav Topic
        EdmEntityType eetTopic = AbstractPebldsTopic.getEntityType(namespace);
        entityTypes.add(eetTopic);
        propertyName = NAV_PROP_TOPICS;

        EdmEntitySet eesTopic = new EdmEntitySet(propertyName, eetTopic);
        entitySets.add(eesTopic);

        lAssoc = new ArrayList<EdmAssociationSet>();
        assocList = new ArrayList<EdmAssociation>();

        assocName = String.format("FK_%s_%s", propertyName, COLLECTION_NAME);
        EdmAssociation assocTopic = new EdmAssociation(namespace, null,
                assocName,
                new EdmAssociationEnd(COLLECTION_NAME, eet, EdmMultiplicity.ZERO_TO_ONE),
                new EdmAssociationEnd(propertyName, eetTopic, EdmMultiplicity.MANY));

        lAssoc.add(new EdmAssociationSet(assocName, assocTopic,
                new EdmAssociationSetEnd(new EdmAssociationEnd(COLLECTION_NAME, eet, EdmMultiplicity.ZERO_TO_ONE), ees),
                new EdmAssociationSetEnd(new EdmAssociationEnd(propertyName, eetTopic, EdmMultiplicity.MANY), eesTopic)));

        np = new EdmNavigationProperty(propertyName, assocTopic, assocTopic.end1, assocTopic.end2);

        eet.navigationProperties.add(np);

        assocList.addAll(Enumerable.create(assocTopic).toList());
